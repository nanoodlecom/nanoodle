#!/usr/bin/env node
// Video extra-image roles (PR #378): last_image / end-frame morph and reference-image arrays.
//
...
//   last — VIDEO_LAST_FRAME_FAMILIES (/^minimax-h3$/i). Day-one catalog gap: the id ships
//     advertising only duration + aspect_ratio, but last_image is live-verified (2026-07-31).
...
// ---- A. family-table twins ------------------------------------------------
{
  ...
  const LAST_YES = ["minimax-h3", "Minimax-H3", "MINIMAX-H3"];
  const LAST_NO = ["minimax-hailuo", "hailuo-02", "seedance-2.0", "luma-ray", "kling-v2", ""];
...
console.log("\n✓ video-image-roles: last_image family fallback + ref pricing/cap twins agree; play payload honors authored wires and drops only when the model is known-incapable.");
