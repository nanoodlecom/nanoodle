...
test('preserves FIBO size/model, H3 cinematic still, Omni version, Fable, BiRefNet, InfiniteTalk, SAM 3, P-Image Upscale, deslop venice-uncensored, H3 Max Multi Angle, character-sprites, transparent-brand-sticker, remove-packaging-text, h3-identity-restyle, ideogram-v4-instant-poster, volt-vector-mark, mai-pack-type, Crystal video upscale, Night-ride SFX, SenseNova dispatch, Grok Imagine 1.5 still and P-Video rewrite pin regressions', async () => {
  ...
  assert.equal(galleryRegressions([]).length, 24);
  ...
  pins[21].id = 'ideogram-v4-instant';
  pins[22].id = 'minimax-h3/image-to-video-spicy';
  pins[23].id = 'clarity-ai/crystal-video-upscaler';
  assert.equal(galleryRegressions(pins).length, 24);
