...
| `google/gemini-omni-flash` | marketing `/api/models` omits `supportsLastImage`; video-models tags have no `first-last-frame` — **not** a last-frame model; do not send `last_image` | `duration`, `aspect_ratio` |
...
`first-last-frame` tag (2026-09-01 catalog GET). v1 is **not** on the allowlist: it advertises
neither the flag nor the tag, so a family-pair guess would send a charged extra image the model
never claimed to take.
