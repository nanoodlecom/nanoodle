...
| · 16 | `wire-routing.mjs` — readable edges (`direct` / `elbow` / `around`); Route wires / Mess up (`?product=16`); `scripts/check-next-action-wire-routing.mjs` |

Learned `.bin` weights stay **gitignored**; tests load inline float fixtures.
Real-editor soft tips: `editor-surface.mjs` mounts in `index.html` (`?product=1`…`6`, `16`).
...
node scripts/check-next-action-wire-routing.mjs
