# vendor/next-action — schema · bake · frequency · ring · cold-start · learned · port-facing
...
| · 14 | `port-facing.mjs` — discrete facing left/right/up/down (`?product=14`); Apply / Scramble |

Learned `.bin` weights stay **gitignored**; tests load inline float fixtures.
Real-editor soft tips: `editor-surface.mjs` mounts in `index.html` (`?product=1`…`6`, `14`).
...
node scripts/check-next-action-port-facing.mjs
