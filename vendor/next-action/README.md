...
| · 18 | `aesthetic-pack.mjs` — multi-select polish / mess (`?product=18`); `scripts/check-next-action-aesthetic-pack.mjs` |

Learned `.bin` weights stay **gitignored**; tests load inline float fixtures.
Real-editor soft tips: `editor-surface.mjs` mounts in `index.html` (`?product=1`…`6`, `18`).
...
node scripts/check-next-action-aesthetic-pack.mjs
